...
<head>
    <meta charset="UTF-8">
    <title>[DEBUG] Debug Edit Product</title>
...
<body>
    <h1>[DEBUG] Debug: Botón "Editar Producto"</h1>
    
    <div class="log">
        <h3>Paso 1: Obtener Token</h3>
        <button onclick="getToken()" class="button">[KEY] Get Token</button>
    ...
    <script>
        ...
        async function getToken() {
            try {
                ...
                if (response.data.success) {
                    ...
                    log('tokenResult', '[SUCCESS] Token obtenido: ' + authToken.substring(0, 50) + '...', 'success');
                    
                    loadProducts();
                } else {
                    log('tokenResult', '[ERROR] Error: ' + response.data.error, 'error');
                }
            } catch (error) {
                log('tokenResult', '[ERROR] Error: ' + error.message, 'error');
            }
        }
        ...
        async function loadProducts() {
            try {
                ...
                if (response.data.success) {
                    ...
                    log('tokenResult', '[SUCCESS] Productos cargados: ' + userProducts.length, 'success');
                    
                    document.getElementById('productsSection').style.display = 'block';
                    document.getElementById('testSection').style.display = 'block';
                    renderProducts();
                } else {
                    log('tokenResult', '[ERROR] Error cargando productos: ' + response.data.error, 'error');
                }
            } catch (error) {
                log('tokenResult', '[ERROR] Error: ' + error.message, 'error');
            }
        ...
        function testEditProduct(projectId, productId) {
            log('testResults', '[REFRESH] Testing editProduct(' + projectId + ', ' + productId + ')');
            ...
            fetch(editURL, {
                ...
                log('testResults', '[DATA] Status: ' + response.status + ' (' + response.statusText + ')', 
                ...
                log('testResults', '[ERROR] Error: ' + error.message, 'error');
            });
        }
    </script>
</body>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                <div class="bg-green-100 p-6 rounded-lg">
                    <h2 class="text-xl font-bold text-green-800 mb-4">[SUCCESS] Problemas Resueltos</h2>
                    <ul class="space-y-2 text-sm text-green-700">
                        <li>[SUCCESS] Error "Unexpected token '<'" eliminado</li>
                        <li>[SUCCESS] Dashboard.js limpiado (de 8577 → 387 líneas)</li>
                        <li>[SUCCESS] Funciones duplicadas eliminadas</li>
                        <li>[SUCCESS] Event listeners conflictivos corregidos</li>
                        <li>[SUCCESS] JavaScript se carga sin errores sintácticos</li>
                        <li>[SUCCESS] Autenticación y redirección funcionan</li>
                    </ul>
                </div>

                <div class="bg-blue-100 p-6 rounded-lg">
                    <h2 class="text-xl font-bold text-blue-800 mb-4">[TOOL] Correcciones Técnicas</h2>
                    <ul class="space-y-2 text-sm text-blue-700">
                        <li>📝 Función <code>renderMyProductsList()</code> corregida</li>
                        <li>[REFRESH] Usa <code>product.description</code> como título</li>
                        <li>🚫 Elimina conflicto <code>viewProductDetails</code></li>
                        <li>⚡ Optimización de bundle (-36% tamaño)</li>
                        <li>[TARGET] Modal "Ver Detalles" implementado</li>
                        <li>[LOCK] Autenticación JWT validada</li>
                    </ul>
                </div>
            </div>

            <div class="mt-8 bg-gray-100 p-6 rounded-lg">
                <h2 class="text-xl font-bold text-gray-800 mb-4">[DATA] Funcionalidad "Mis Productos" - VERIFICADA</h2>
                
                <div class="bg-white p-4 rounded border-l-4 border-green-500">
                    <h3 class="font-bold text-green-800 mb-2">API Endpoint: <code>/api/private/products</code></h3>
                    <p class="text-sm text-gray-600 mb-3">Estado: <span class="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">[SUCCESS] FUNCIONANDO</span></p>
        ...
        fetch('/api/public/test-generate-token')
            ...
            .then(data => {
                if (data.success) {
                    ...
                    console.log('[SUCCESS] Token de prueba establecido automáticamente');
                    
                    // Mostrar información del token
                    const tokenInfo = document.createElement('div');
                    tokenInfo.className = 'mt-4 p-3 bg-indigo-100 rounded text-sm text-indigo-800';
                    tokenInfo.innerHTML = \`
                        <strong>[LOCK] Token establecido para:</strong> \${data.data.user.email} (\${data.data.user.role})
    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
        ...
        <div class="space-y-4">
            <button onclick="setTestToken()" class="w-full bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                [LOCK] Establecer Token de Investigador Demo
            </button>
    ...
    <script>
        async function setTestToken() {
            try {
                ...
                showResult('[SUCCESS] Token establecido para Dr. Investigador Demo (ID: 2)', 'success');
            } catch (error) {
        ...
        async function testProductsAPI() {
            try {
                ...
                if (!token) {
                    showResult('[ERROR] No hay token. Establece uno primero.', 'error');
                    return;
                }
                ...
                if (response.data.success) {
                    ...
                    showResult(\`[SUCCESS] API funciona. Productos encontrados: \${productos.length}\\n\\n\${productos.map(p => \`- \${p.description || p.product_code}\`).join('\\n')}\`, 'success');
                } else {
                    showResult(\`[ERROR] API Error: \${response.data.error}\`, 'error');
                }
            } catch (error) {
                showResult(\`[ERROR] Error llamando API: \${error.response?.data?.error || error.message}\`, 'error');
            }
        }
        ...
        function clearToken() {
            ...
            showResult('[SUCCESS] Token eliminado', 'info');
        }
...
<body class="bg-gray-100 p-8">
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
        <h1 class="text-2xl font-bold mb-4">[TOOL] Establecer Token de Autenticación</h1>
        ...
        <div class="mt-6">
            <h2 class="text-lg font-semibold mb-2">[START] Acciones Rápidas:</h2>
            <div class="space-y-2">
        ...
        function checkToken() {
            ...
            try {
                ...
                const info = '<strong>Token encontrado:</strong><br>' +
                    ...
                    'Estado: ' + (expTime > now ? '[SUCCESS] Válido' : '[ERROR] Expirado');
...
<body class="bg-gray-100 p-8">
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow">
        <h1 class="text-2xl font-bold mb-4">[TOOL] Diagnóstico Completo de Autenticación</h1>
    ...
    <script>
        ...
        function step1_setToken() {
            log('[DEBUG] PASO 1: ESTABLECIENDO TOKEN');
            localStorage.removeItem('auth_token');
            ...
            try {
                ...
                log('[ERROR] Error decodificando token: ' + e.message);
            }
        }

        function step2_configureAxios() {
            log('[DEBUG] PASO 2: CONFIGURANDO AXIOS');
            if (!authToken) {
                log('[ERROR] ERROR: No hay token disponible. Ejecuta Paso 1 primero.');
                return;
            }
            axios.defaults.headers.common['Authorization'] = 'Bearer ' + authToken;
            ...
            axios.interceptors.request.use(
                function(config) {
                    log('[UP] INTERCEPTOR: Solicitud saliente a ' + config.url);
                    log('[UP] Auth header: ' + (config.headers.Authorization ? 'PRESENTE' : 'AUSENTE'));
                    return config;
                },
                function(error) {
                    log('[ERROR] INTERCEPTOR: Error en solicitud: ' + error.message);
            ...
            axios.interceptors.response.use(
                function(response) {
                    log('[DOWN] INTERCEPTOR: Respuesta recibida - Status: ' + response.status);
                    return response;
                },
                function(error) {
                    log('[ERROR] INTERCEPTOR: Error en respuesta: ' + (error.response ? error.response.status : error.message));
                    return Promise.reject(error);
                }
            );
        ...
        function step3_testAPI() {
            log('[DEBUG] PASO 3: PROBANDO API');
            if (!authToken) {
                log('[ERROR] ERROR: No hay token disponible. Ejecuta Paso 1 primero.');
                return;
            }
            log('[START] Enviando solicitud a: ' + API_BASE + '/private/projects/1');
            
            axios.get(API_BASE + '/private/projects/1')
                .then(function(response) {
                    log('[SUCCESS] ÉXITO: Respuesta recibida');
                    log('[SUCCESS] Status: ' + response.status + ' ' + response.statusText);
                    log('[SUCCESS] Success: ' + response.data.success);
                    if (response.data.success && response.data.data) {
                        log('[SUCCESS] Proyecto cargado: ' + response.data.data.title);
                        log('[SUCCESS] Propietario: ' + response.data.data.owner_name);
                    }
                })
                .catch(function(error) {
                    log('[ERROR] ERROR: Fallo en API');
                    if (error.response) {
                        log('[ERROR] Status: ' + error.response.status);
                        log('[ERROR] Error: ' + (error.response.data ? error.response.data.error : 'Desconocido'));
                    } else {
        ...
        function step4_fullTest() {
            log('[START] INICIANDO TEST COMPLETO...');
            log('='.repeat(50));
            step1_setToken();
            setTimeout(function() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            log('[TOOL] Sistema de diagnóstico cargado');
            log('[TOOL] Axios disponible: ' + (typeof axios !== 'undefined' ? 'SÍ' : 'NO'));
            log('[TOOL] localStorage disponible: ' + (typeof localStorage !== 'undefined' ? 'SÍ' : 'NO'));
            log('');
            log('Instrucciones:');
            ...
            log('4. O usa "[START] Test Completo" para ejecutar todo automáticamente');
            log('');
        });
    </script>
</body>
...
<body class="bg-gray-100 p-8">
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow">
        <h1 class="text-2xl font-bold mb-4">[START] Test Automático de Autenticación</h1>
    ...
    <script>
        ...
        function runAutomaticTest() {
            log('[START] INICIANDO TEST AUTOMÁTICO COMPLETO');
            log('='.repeat(60));
            
            // PASO 1: Establecer token
            log('');
            log('[DEBUG] PASO 1: ESTABLECIENDO TOKEN EN localStorage');
            localStorage.removeItem('auth_token');
            localStorage.setItem('auth_token', TEST_TOKEN);
            const storedToken = localStorage.getItem('auth_token');
            authToken = storedToken;
            ...
            setTimeout(() => {
                log('');
                log('[DEBUG] PASO 2: CONFIGURANDO AXIOS CON TOKEN');
                axios.defaults.headers.common['Authorization'] = 'Bearer ' + authToken;
                const authHeader = axios.defaults.headers.common['Authorization'];
                ...
                axios.interceptors.request.use(
                    function(config) {
                        log('[UP] REQUEST INTERCEPTOR: ' + config.method.toUpperCase() + ' ' + config.url);
                        log('[UP] Auth header presente: ' + (config.headers.Authorization ? 'SÍ' : 'NO'));
                        if (config.headers.Authorization) {
                            log('[UP] Auth value: ' + config.headers.Authorization.substring(0, 20) + '...');
                        }
                        return config;
                    },
                    function(error) {
                ...
                axios.interceptors.response.use(
                    function(response) {
                        log('[DOWN] RESPONSE SUCCESS: Status ' + response.status + ' from ' + response.config.url);
                        return response;
                    },
                    function(error) {
                        log('[ERROR] RESPONSE ERROR: Status ' + (error.response ? error.response.status : 'N/A') + ' - ' + error.message);
                        return Promise.reject(error);
                    }
                );
                ...
                setTimeout(() => {
                    log('');
                    log('[DEBUG] PASO 3: PROBANDO ENDPOINT DE API');
                    log('[START] Haciendo solicitud GET a: ' + API_BASE + '/private/projects/1');
                    
                    axios.get(API_BASE + '/private/projects/1')
                        .then(function(response) {
                            log('');
                            log('[SUCCESS] ¡ÉXITO COMPLETO! API RESPONDIÓ CORRECTAMENTE');
                            log('[SUCCESS] Status HTTP: ' + response.status + ' ' + response.statusText);
                            log('[SUCCESS] Response success: ' + response.data.success);
                            
                            if (response.data.success && response.data.data) {
                                log('[SUCCESS] Proyecto cargado exitosamente:');
                        ...
                        .catch(function(error) {
                            log('');
                            log('[ERROR] ERROR EN API - AQUÍ ESTÁ EL PROBLEMA:');
                            if (error.response) {
                                log('[ERROR] Status HTTP: ' + error.response.status);
                                log('[ERROR] Mensaje: ' + (error.response.data ? JSON.stringify(error.response.data) : 'Sin mensaje'));
                                
                                if (error.response.status === 401) {
                                    log('[ERROR] DIAGNÓSTICO: Token no enviado o inválido');
                                    log('[ERROR] Verificar que axios.defaults.headers.common esté configurado');
                                } else if (error.response.status === 403) {
                                    log('[ERROR] DIAGNÓSTICO: Token válido pero sin permisos');
                                } else {
                                    log('[ERROR] DIAGNÓSTICO: Error diferente - revisar backend');
                                }
                            } else {
                                log('[ERROR] Error de red: ' + error.message);
                                log('[ERROR] DIAGNÓSTICO: Problema de conectividad o CORS');
                            }
                        });
                }, 1000);
            }, 1000);
        ...
        function goToEditPage() {
            log('');
            log('[START] NAVEGANDO A PÁGINA DE EDICIÓN...');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            log('[TOOL] Sistema de test automático cargado');
            log('[TOOL] Dependencias verificadas:');
            log('   - Axios: ' + (typeof axios !== 'undefined' ? '✓ Disponible' : '[ERROR] No disponible'));
            log('   - localStorage: ' + (typeof localStorage !== 'undefined' ? '✓ Disponible' : '[ERROR] No disponible'));
            log('');
...
<body class="bg-gray-100 p-8">
    ...
    <script>
        ...
        document.getElementById('loginForm').addEventListener('submit', async function(e) {
            ...
            log('[START] === INICIANDO LOGIN COMPLETO ===');
            log('📧 Email: ' + email);
            log('[KEY] Password: ***' + password.length + ' chars***');
            
            try {
                log('[WEB] Enviando solicitud POST a: ' + API_BASE + '/auth/login');
                ...
                if (response.data && response.data.success) {
                    ...
                    log('[SUCCESS] LOGIN EXITOSO para ' + user.full_name);
                    log('🎫 Token recibido: ' + token.substring(0, 30) + '...');
                    
                    // ESTA ES LA LÍNEA CRÍTICA - GUARDAR TOKEN
                    log('[SAVE] Guardando token en localStorage con clave "auth_token"...');
                    localStorage.setItem('auth_token', token);
                    
                    // Verificar que se guardó
                    const storedToken = localStorage.getItem('auth_token');
                    log('[SUCCESS] Token verificado en localStorage: ' + (storedToken ? 'PRESENTE' : 'AUSENTE'));
                    
                    // Configurar axios
                    axios.defaults.headers.common['Authorization'] = 'Bearer ' + token;
                    log('[SUCCESS] Cabeceras de axios configuradas');
                    
                    log('🎉 LOGIN COMPLETO Y TOKEN GUARDADO EXITOSAMENTE');
                    log('[TARGET] Ahora puedes probar las acciones post-login');
        ...
        function testAPI() {
            ...
            const token = localStorage.getItem('auth_token');
            if (!token) {
                log('[ERROR] No hay token - haz login primero');
                return;
            }
            
            log('[START] Enviando GET a /api/private/projects/1');
            
            axios.get('/api/private/projects/1')
                .then(function(response) {
                    log('[SUCCESS] API respondió exitosamente: Status ' + response.status);
                    log('[SUCCESS] Success: ' + response.data.success);
                    if (response.data.data) {
                        log('[SUCCESS] Proyecto: ' + response.data.data.title);
                        log('[SUCCESS] Propietario: ' + response.data.data.owner_name);
                    }
                })
                .catch(function(error) {
                    log('[ERROR] ERROR en API: Status ' + (error.response ? error.response.status : 'N/A'));
                    log('[ERROR] Mensaje: ' + (error.response ? JSON.stringify(error.response.data) : error.message));
                });
        }
        ...
        function goToEdit() {
            ...
            log('[SUCCESS] Token presente - navegando...');
            setTimeout(() => {
        ...
        // Log inicial
        log('[TOOL] Sistema cargado');
        log('📝 Credenciales precargadas para prueba rápida');
        log('[TARGET] Presiona "Hacer Login Completo" para comenzar');
    </script>
</body>
</html>`);
})
...
<body>
    <div class="container">
        <h1>[TOOL] DEBUG LOGIN AUTOMÁTICO</h1>
        <div id="logs"></div>
    </div>
    ...
    <script>
        ...
        async function executeLoginWithDebugger() {
            log('[START] === INICIANDO LOGIN AUTOMÁTICO CON DEPURACIÓN ===');
            ...
            try {
                ...
                if (response.data && response.data.success) {
                    ...
                    log('[DEBUG] === PUNTO DE RUPTURA ALCANZADO ===', 'warning');
                    debugger; // <-- EL CÓDIGO SE DETENDRÁ AQUÍ
                    
                    const user = response.data.data.user;
                    const token = response.data.data.token;
                    
                    log('[SUCCESS] LOGIN EXITOSO para ' + user.full_name, 'success');
                    log('🎫 Token generado: ' + token.substring(0, 30) + '...', 'success');
                    
                    // INVESTIGACIÓN DETALLADA DEL TOKEN
                    log('[DEBUG] === INVESTIGACIÓN DETALLADA ===', 'warning');
                    log('[DEBUG] typeof token: ' + typeof token);
                    log('[DEBUG] token.length: ' + (token ? token.length : 'N/A'));
                    log('[DEBUG] token es string: ' + (typeof token === 'string' ? 'SÍ' : 'NO'));
                    log('[DEBUG] token truthy: ' + (token ? 'SÍ' : 'NO'));
                    log('[DEBUG] response.data estructura: ' + JSON.stringify(Object.keys(response.data)));
                    log('[DEBUG] response.data.data estructura: ' + JSON.stringify(Object.keys(response.data.data)));
                    
                    // INVESTIGACIÓN DE LOCALSTORAGE
                    log('[DEBUG] === INVESTIGACIÓN DE LOCALSTORAGE ===', 'warning');
                    log('[DEBUG] localStorage disponible: ' + (typeof localStorage !== 'undefined' ? 'SÍ' : 'NO'));
                    log('[DEBUG] localStorage.setItem función: ' + (typeof localStorage.setItem === 'function' ? 'SÍ' : 'NO'));
                    ...
                    try {
                        ...
                        log('[ERROR] Error limpiando localStorage: ' + e.message, 'error');
                    }
                    
                    // Guardar token y redirigir
                    try {
                        log('[DEBUG] DEPURACIÓN: Intentando guardar token...');
                        log('[DEBUG] Token variable: ' + (token ? 'PRESENTE (' + token.length + ' chars)' : 'AUSENTE'));
                        log('[DEBUG] localStorage disponible: ' + (typeof localStorage !== 'undefined' ? 'SÍ' : 'NO'));
                        
                        // LÍNEA CRÍTICA
                        log('[TARGET] === EJECUTANDO localStorage.setItem() ===', 'warning');
                        localStorage.setItem('auth_token', token);
                        log('[SAVE] localStorage.setItem() ejecutado SIN ERRORES', 'success');
                        
                        // Verificar inmediatamente si se guardó
                        const verificacion = localStorage.getItem('auth_token');
                        log('[DEBUG] VERIFICACIÓN INMEDIATA: Token en localStorage: ' + (verificacion ? 'PRESENTE (' + verificacion.length + ' chars)' : 'AUSENTE'), verificacion ? 'success' : 'error');
                        
                        // Comparar tokens
                        if (verificacion && token) {
                            log('[DEBUG] Los tokens coinciden: ' + (verificacion === token ? 'SÍ' : 'NO'), verificacion === token ? 'success' : 'error');
                        }
                        
                        // Verificar todas las claves en localStorage
                        log('[DEBUG] Todas las claves en localStorage: ' + Object.keys(localStorage).join(', '));
                        
                        log('[SUCCESS] === DEPURACIÓN COMPLETA - REVISAR RESULTADOS ===', 'success');
                        
                    } catch (storageError) {
                        log('[ERROR] ERROR CRÍTICO guardando token: ' + storageError.message, 'error');
                        log('[ERROR] Stack: ' + storageError.stack, 'error');
                    }
                    
                } else {
                    const errorMsg = (response.data && response.data.error) || 'Error desconocido';
                    log('[ERROR] Login fallido: ' + errorMsg, 'error');
                }
                
            } catch (error) {
                log('[ERROR] ERROR EN LOGIN: ' + error.message, 'error');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            log('[TOOL] Sistema de debug cargado');
...
<body>
    <div class="container">
        <h1>[TOOL] TEST FLUJO COMPLETO: Login → Token → Redirect</h1>
    ...
    <script>
        ...
        async function executeFullFlow() {
            // PASO 1: LOGIN
            log('[START] INICIANDO LOGIN...', 'loginLogs');
            ...
            try {
                ...
                log('[SUCCESS] Respuesta del servidor: Status ' + response.status, 'loginLogs', 'success');
                
                if (response.data && response.data.success) {
                    const token = response.data.data.token;
                    log('[SUCCESS] Token recibido: ' + token.substring(0, 30) + '...', 'loginLogs', 'success');
                    ...
                    // Guardar token
                    localStorage.setItem('auth_token', token);
                    log('[SAVE] Token guardado con clave "auth_token"', 'loginLogs', 'success');
                    
                    // PASO 2: VERIFICACIÓN INMEDIATA
                    setTimeout(() => verifyToken(), 1000);
                } else {
                    log('[ERROR] Login falló: ' + (response.data.error || 'Error desconocido'), 'loginLogs', 'error');
                }
                
            } catch (error) {
                log('[ERROR] Error en login: ' + error.message, 'loginLogs', 'error');
            }
        }

        function verifyToken() {
            log('[DEBUG] VERIFICANDO TOKEN EN LOCALSTORAGE...', 'tokenLogs');
            ...
            if (token) {
                // Configurar axios
                axios.defaults.headers.common['Authorization'] = 'Bearer ' + token;
                log('[SUCCESS] Cabeceras de axios configuradas', 'tokenLogs', 'success');
                
                // Verificar estructura del token
                try {
                    const parts = token.split('.');
                    const payload = JSON.parse(atob(parts[1]));
                    log('[SUCCESS] Token decodificado - Usuario: ' + payload.email, 'tokenLogs', 'success');
                    log('[SUCCESS] Rol: ' + payload.role, 'tokenLogs', 'success');
                    log('[SUCCESS] Expira: ' + new Date(payload.exp * 1000).toLocaleString(), 'tokenLogs', 'success');
                } catch (e) {
                    log('[WARNING] Error decodificando token: ' + e.message, 'tokenLogs', 'warning');
                }
                
                // PASO 3: TEST API
                setTimeout(() => testAPI(), 1000);
            } else {
                log('[ERROR] No se puede continuar sin token', 'tokenLogs', 'error');
            }
        }
        ...
        async function testAPI() {
            ...
            try {
                log('[START] Enviando GET a /api/private/projects/1', 'apiLogs');
                
                const response = await axios.get('/api/private/projects/1');
                
                log('[SUCCESS] API respondió: Status ' + response.status, 'apiLogs', 'success');
                log('[SUCCESS] Success: ' + response.data.success, 'apiLogs', 'success');
                
                if (response.data.success && response.data.data) {
                    log('[SUCCESS] Proyecto cargado: ' + response.data.data.title, 'apiLogs', 'success');
                    log('[SUCCESS] Propietario: ' + response.data.data.owner_name, 'apiLogs', 'success');
                    
                    // ÉXITO TOTAL
                    log('🎉 ¡FLUJO COMPLETO EXITOSO!', 'navLogs', 'success');
                    log('[SUCCESS] Login funcionó', 'navLogs', 'success');
                    log('[SUCCESS] Token se guardó', 'navLogs', 'success');
                    log('[SUCCESS] API respondió correctamente', 'navLogs', 'success');
                    log('[SUCCESS] Listo para navegar a página de edición', 'navLogs', 'success');
                    
                    document.getElementById('editBtn').style.display = 'block';
                } else {
                    log('[ERROR] API devolvió error: ' + JSON.stringify(response.data), 'apiLogs', 'error');
                }
                
            } catch (error) {
                log('[ERROR] Error en API: ' + error.message, 'apiLogs', 'error');
                if (error.response) {
                    log('[ERROR] Status: ' + error.response.status, 'apiLogs', 'error');
                    log('[ERROR] Data: ' + JSON.stringify(error.response.data), 'apiLogs', 'error');
                }
                
                log('[DEBUG] DIAGNÓSTICO: El problema está aquí ↑', 'navLogs', 'warning');
            }
        }

        function goToEdit() {
            log('[START] NAVEGANDO A PÁGINA DE EDICIÓN...', 'navLogs');
            log('[SUCCESS] Token en localStorage antes de navegar: ' + (localStorage.getItem('auth_token') ? 'PRESENTE' : 'AUSENTE'), 'navLogs');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            log('[TOOL] Sistema cargado - Iniciando flujo en 2 segundos...', 'loginLogs');
    ...
    <div class="p-8">
        <div class="max-w-6xl mx-auto space-y-6">
            ...
            <div class="bg-muted/50 border border-border rounded-lg p-4">
                <h3 class="font-semibold mb-2">[DEBUG] Debug Info</h3>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>[TOOL] Test Dashboard Monitoreo - CTeI Manager</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet">
    <link href="/static/styles.css" rel="stylesheet">
    <script src="https://cdn.jsdelivr.net/npm/axios@1.6.0/dist/axios.min.js"></script>
...
<body class="bg-gray-100">
    <div class="container mx-auto p-8">
        <h1 class="text-3xl font-bold mb-6">[TOOL] Test Dashboard de Monitoreo</h1>
        <div class="space-y-4">
            <div class="bg-white p-6 rounded-lg shadow">
        ...
        async function testLogin() {
            try {
                ...
                if (response.data.success) {
                    ...
                    alert('[SUCCESS] Login exitoso! Token guardado. Ir al dashboard?');
                    if (confirm('¿Ir al dashboard principal?')) {
                        window.location.href = '/dashboard';
                    }
                } else {
                    alert('[ERROR] Error en login: ' + response.data.error);
                }
            } catch (error) {
                console.error('Error:', error);
                alert('[ERROR] Error de conexión: ' + error.message);
            }
        }
        
        async function testMonitoringAPI() {
            if (!adminToken) {
                alert('[WARNING] Necesitas hacer login primero');
                return;
            }
            ...
            try {
                ...
                resultDiv.innerHTML = \`
                    <h3 class="font-semibold text-green-600">[SUCCESS] API Response:</h3>
                ...
                document.getElementById('apiResult').innerHTML = \`
                    <h3 class="font-semibold text-red-600">[ERROR] API Error:</h3>
                    <p>\${error.message}</p>
                \`;
        ...
        async function testDashboard() {
            ...
            container.innerHTML = \`
                <div class="bg-blue-50 p-4 rounded">
                    <h3 class="font-semibold mb-2">[START] Simulación Dashboard Monitoreo</h3>
                    <div class="space-y-2">
                        <div class="bg-white p-2 rounded">[DATA] Métricas del Sistema</div>
                        <div class="bg-white p-2 rounded">[CHART] Líneas de Acción</div>
                        <div class="bg-white p-2 rounded">[WARNING] Proyectos de Atención</div>
                        <div class="bg-white p-2 rounded">[REFRESH] Auto-refresh: 30s</div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>[TOOL] Login Limpio - CTeI Manager</title>
    <script src="https://cdn.jsdelivr.net/npm/axios@1.6.0/dist/axios.min.js"></script>
    <style>
...
<body>
    <div class="container">
        <h1>[TOOL] Login Directo - Sin Interferencias</h1>
        ...
        <form id="loginForm">
            ...
            <button type="submit">[START] PROBAR LOGIN</button>
        </form>
    ...
    <script>
        ...
        function showResult(success, message, data = null) {
            ...
            if (success) {
                ...
                result.innerHTML = '<h3>[SUCCESS] ¡LOGIN EXITOSO!</h3><p>' + message + '</p>' + 
                    (data ? '<pre>' + JSON.stringify(data, null, 2) + '</pre>' : '');
            } else {
                result.className = 'result error';
                result.innerHTML = '<h3>[ERROR] Login Fallido</h3><p>' + message + '</p>';
            }
        }
        ...
        document.getElementById('loginForm').addEventListener('submit', async function(e) {
            ...
            log('[START] === INICIANDO LOGIN ===');
            log('📧 Email: ' + email);
            log('[KEY] Password: ' + (password ? '***SET***' : 'EMPTY'));
            log('[WEB] API Base: ' + API_BASE);
            log('📦 Axios: ' + (typeof axios !== 'undefined' ? 'DISPONIBLE' : 'NO DISPONIBLE'));
            
            if (!email || !password) {
                log('[ERROR] Campos vacíos');
                showResult(false, 'Por favor ingresa email y contraseña');
                return;
            }
            
            try {
                log('[WEB] Enviando petición POST a: ' + API_BASE + '/auth/login');
                ...
                if (response.data && response.data.success) {
                    ...
                    try {
                        ...
                        localStorage.setItem('auth_token', token);
                        log('[SAVE] localStorage.setItem() ejecutado');
                        
                        // Verificar inmediatamente si se guardó
                        const verificacion = localStorage.getItem('auth_token');
                        log('[DEBUG] VERIFICACIÓN INMEDIATA: Token en localStorage: ' + (verificacion ? 'PRESENTE (' + verificacion.length + ' chars)' : 'AUSENTE'));
                        
                        log('[REFRESH] Redirigiendo al dashboard...');
                        setTimeout(function() {
                            window.location.href = '/dashboard';
                        }, 2000);
                        
                    } catch (storageError) {
                        log('[WARNING] Error guardando token: ' + storageError.message);
                    }
        ...
        // Inicialización
        log('[START] Página cargada correctamente');
        log('[WEB] URL actual: ' + window.location.href);
...
<body class="bg-gray-100 p-8">
    <div class="max-w-md mx-auto bg-white p-6 rounded shadow">
        <h1 class="text-2xl font-bold mb-4">[DEBUG] Debug Login Frontend</h1>
        
        <!-- Test admin@test.com -->
        <div class="mb-6 p-4 border rounded">
            <h2 class="text-lg font-semibold mb-2">[KEY] Admin Login Test</h2>
        ...
        <div id="logs" class="mt-4 p-3 bg-gray-100 rounded text-xs">
            <h3 class="font-bold mb-2">[INFO] Logs:</h3>
    ...
    <script>
        ...
        function showResult(success, message, data = null) {
            ...
            if (success) {
                ...
                resultDiv.innerHTML = \`
                    <h3 class="font-bold">[SUCCESS] LOGIN EXITOSO</h3>
                    <p>\${message}</p>
                ...
                resultDiv.innerHTML = \`
                    <h3 class="font-bold">[ERROR] LOGIN FALLIDO</h3>
                    <p>\${message}</p>
                \`;
            }
        ...
        async function performLogin(email, password) {
            log(\`[START] Iniciando login para: \${email}\`);
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    log(\`[SUCCESS] Token guardado en localStorage\`);
        ...
        log('[START] Debug page loaded');
        log(\`[WEB] Current URL: \${window.location.href}\`);
        ...
        async function loadSiteConfig() {
            try {
                ...
                if (response.data.success && response.data.data.logo_url) {
                    ...
                    console.log('[SUCCESS] Logo personalizado cargado (solo imagen):', logoUrl);
                } else {
                    ...
                    console.log('[SUCCESS] Logo por defecto de CODECTI CHOCÓ cargado (solo imagen)');
                }
            } catch (error) {
                ...
                logoContainer.innerHTML = \`
                    ...
                    >
                \`;
                console.warn('[WARNING] Error cargando configuración, usando logo fallback (solo imagen):', error);
            }
        }
...
// ========================================
// [TARGET] NUEVAS RUTAS DINÁMICAS PARA PÁGINAS DEDICADAS
// ========================================
  ...
  return c.html(`
    ...
    <body class="level-0 bg-background">
        ...
        <script>
            ...
            console.log('[LOCK] Página de login inicializada');
        </script>
    </body>
    </html>
  `);
    ...
    <body class="bg-background text-foreground min-h-screen">
        <div id="app">
            ...
            <div class="p-8 text-center">
                <div class="text-lg">[REFRESH] Cargando dashboard...</div>
            </div>
        </div>
        ...
        <script>
            ...
            console.log('[SUCCESS] Dashboard JavaScript cargado');
...
app.get('/dashboard/productos/:id/editar', async (c) => {
  ...
  return c.html(`
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Editar Producto Científico - CTeI-Manager</title>
        <script src="https://cdn.tailwindcss.com"></script>
        <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet">
        <link href="/static/styles.css" rel="stylesheet">
        <script src="https://cdn.jsdelivr.net/npm/axios@1.6.0/dist/axios.min.js"></script>
    ...
    <body class="level-0 bg-background text-foreground">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="text-muted-foreground">Mis Productos</span>
                            </div>
                        </li>
                        <li aria-current="page">
                            <div class="flex items-center">
                                <i class="fas fa-chevron-right text-muted-foreground mx-2"></i>
                                <span id="page-title" class="text-foreground font-medium">Editando Producto...</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                ...
                <div class="flex justify-between items-center">
                    <div>
                        <h1 id="header-title" class="text-3xl font-bold text-foreground mb-2">
                            ...
                            Editar Producto Científico
                        </h1>
                        <p class="text-muted-foreground">
                            Modifica los detalles de tu producto científico y gestiona sus archivos
                        </p>
                    </div>
                    <div class="flex items-center space-x-3">
        ...
        <div class="edit-layout">
                    ...
                    <form id="edit-product-form">
                        <div class="form-field">
                            ...
                            <textarea id="product-description" name="description" class="form-textarea" required
                                placeholder="Describe detalladamente tu producto científico..."></textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="form-field">
                                ...
                                <input type="text" id="product-code" name="product_code" class="form-input" required
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="form-field">
                                <label for="product-doi" class="form-label">
                                    <i class="fas fa-link text-primary mr-2"></i>
                                    DOI
                                </label>
                                <input type="text" id="product-doi" name="doi" class="form-input"
                            ...
                            <div class="form-field">
                                ...
                                <input type="url" id="product-url" name="url" class="form-input"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="form-field">
                                <label for="product-journal" class="form-label">
                                    <i class="fas fa-book text-primary mr-2"></i>
                                    Revista/Editorial
                                </label>
                                <input type="text" id="product-journal" name="journal" class="form-input"
                                    placeholder="Nature, Science, IEEE, etc.">
                            </div>
                            
                            <div class="form-field">
                                <label for="product-publication-date" class="form-label">
                                    <i class="fas fa-calendar text-primary mr-2"></i>
                                    Fecha de Publicación
                                </label>
                                <input type="date" id="product-publication-date" name="publication_date" class="form-input">
                            </div>
                        </div>
                        
                        <div class="form-field">
                            <label for="product-impact-factor" class="form-label">
                                <i class="fas fa-star text-primary mr-2"></i>
                                Factor de Impacto
                            </label>
                            <input type="number" id="product-impact-factor" name="impact_factor" step="0.001" min="0" class="form-input"
                                placeholder="2.5">
                        </div>
            ...
            <div class="sidebar-column">
                ...
                <div class="panel">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center">
                            <input type="radio" id="visibility-public" name="visibility" value="public" class="mr-3">
                            <label for="visibility-public" class="flex-1 cursor-pointer">
                                <div class="font-medium">Público</div>
                                <div class="text-xs text-muted-foreground">Visible para todos los usuarios</div>
                            </label>
                        </div>
                        <div class="flex items-center">
                            <input type="radio" id="visibility-private" name="visibility" value="private" class="mr-3" checked>
                            <label for="visibility-private" class="flex-1 cursor-pointer">
                                <div class="font-medium">Privado</div>
                                <div class="text-xs text-muted-foreground">Solo visible para colaboradores</div>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div class="panel">
                    ...
                    <div class="text-xs text-muted-foreground mt-2">
                        <i class="fas fa-info-circle mr-1"></i>
                        Los archivos se almacenan de forma segura y solo son accesibles por colaboradores del producto.
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="loading-overlay" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
            <div class="bg-card p-6 rounded-lg flex items-center gap-4">
                ...
                <span class="text-foreground">Cargando producto...</span>
            </div>
        </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', async () => {
                console.log('=== INICIANDO PÁGINA DE EDICIÓN DE PRODUCTO ===');
            ...
            async function loadProduct() {
                try {
                    const response = await axios.get(\`\${API_BASE}/private/products/\${PRODUCT_ID}\`);
                    ...
                    if (response.data.success) {
                        ...
                        throw new Error(response.data.message || 'Error al cargar el producto');
                    }
                } catch (error) {
                    console.error('[ERROR] Error cargando producto:', error);
                    ...
                    if (error.response) {
                        ...
                        switch (status) {
                            case 401:
                                ...
                                break;
                            case 403:
                                showSpecificError('permission', 'No tienes permisos para editar este producto');
                                break;
                            case 404:
                                showSpecificError('notfound', 'Producto no encontrado');
                                break;
                            default:
            ...
            function populateForm() {
                ...
                document.getElementById('product-code').value = currentProduct.product_code || '';
                document.getElementById('product-type').value = currentProduct.product_type || '';
                document.getElementById('product-doi').value = currentProduct.doi || '';
                document.getElementById('product-url').value = currentProduct.url || '';
                document.getElementById('product-journal').value = currentProduct.journal || '';
                document.getElementById('product-publication-date').value = currentProduct.publication_date || '';
                document.getElementById('product-impact-factor').value = currentProduct.impact_factor || '';
                
                // Visibilidad
                const isPublic = currentProduct.is_public === 1 || currentProduct.is_public === true;
            ...
            function updatePageTitle() {
                if (currentProduct && currentProduct.description) {
                    const shortTitle = currentProduct.description.substring(0, 50) + '...';
                    pageTitle.textContent = \`Editando: \${shortTitle}\`;
                    headerTitle.innerHTML = \`
                        <i class="fas fa-edit text-primary mr-3"></i>
                        Editar Producto: \${shortTitle}
                    \`;
                } else {
                    pageTitle.textContent = \`Error al cargar producto (ID: \${PRODUCT_ID})\`;
                }
            }
            ...
            async function loadProductTypes() {
                try {
                    ...
                    if (response.data.success) {
                        ...
                        if (currentProduct && currentProduct.product_type) {
                            typeSelect.value = currentProduct.product_type;
                        }
                    }
                } catch (error) {
            ...
            async function loadProductFiles() {
                try {
                    const response = await axios.get(\`\${API_BASE}/private/products/\${PRODUCT_ID}/files\`);
                    
                    if (response.data.success) {
                        productFiles = response.data.data.files || [];
                        renderFilesList();
                    }
                } catch (error) {
            ...
            function renderFilesList() {
                const filesList = document.getElementById('product-files-list');
                if (!filesList) return;
                ...
                const filesHTML = productFiles.map(file => {
            ...
            async function handleFormSubmit(event) {
                ...
                try {
                    ...
                    const productData = {
                        ...
                        impact_factor: formData.get('impact_factor') || null,
                        is_public: formData.get('visibility') === 'public'
                    };
                    
                    const response = await axios.put(\`\${API_BASE}/private/products/\${PRODUCT_ID}\`, productData);
                    ...
                    if (error.response) {
                        ...
                        switch (status) {
                            case 403:
                                errorMessage = data.details || data.error || 'No tienes permisos para editar este producto';
                                break;
                            case 404:
            ...
            async function handleFileUpload(files) {
                ...
                try {
                    for (let i = 0; i < files.length; i++) {
                        ...
                        try {
                            const response = await axios.post(\`\${API_BASE}/private/products/\${PRODUCT_ID}/files\`, formData, {
                                headers: {
            ...
            function showAuthError() {
                pageTitle.textContent = \`Acceso Restringido - Producto ID: \${PRODUCT_ID}\`;
                showSpecificError('auth', 'Sesión Requerida', 
                    'Para editar productos necesitas iniciar sesión primero.');
            }
            
            function showSpecificError(type, title, message = '') {
                hideLoading();
                // Implementar mostrar error específico
                alert(\`\${title}: \${message}\`);
            }
            ...
            function downloadFile(fileId) {
                window.open(\`\${API_BASE}/private/products/\${PRODUCT_ID}/files/\${fileId}/download\`, '_blank');
            }
            ...
            async function deleteFile(fileId) {
                ...
                try {
                    const response = await axios.delete(\`\${API_BASE}/private/products/\${PRODUCT_ID}/files/\${fileId}\`);
...
app.get('/dashboard-mejorado', (c) => {
  return c.html(`
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>[START] Dashboard de Monitoreo - Clase Mundial</title>
        <script src="https://cdn.tailwindcss.com"></script>
        <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet">
        <link href="/static/styles.css" rel="stylesheet">
    </head>
    <body class="bg-background text-foreground min-h-screen">
        ...
        <script>
            ...
            // Inicializar
            document.addEventListener('DOMContentLoaded', function() {
                console.log('[START] Dashboard de Clase Mundial inicializado');
...
app.get('/dashboard-simple', (c) => {
  return c.html(`
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>[TOOL] Dashboard Simple - Debug</title>
        <script src="https://cdn.tailwindcss.com"></script>
        <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet">
        <script src="https://cdn.jsdelivr.net/npm/axios@1.6.0/dist/axios.min.js"></script>
    </head>
    <body class="bg-gray-100">
        <div id="app" class="p-8">
            <div class="max-w-4xl mx-auto">
                <h1 class="text-3xl font-bold mb-6">[TOOL] Dashboard Simple - Test</h1>
                
                <!-- Status Panel -->
                <div class="bg-white p-6 rounded-lg shadow mb-6">
                    <h2 class="text-xl font-semibold mb-4">Estado del Sistema</h2>
                    <div id="status-info">
                        <p>[REFRESH] Verificando...</p>
                    </div>
                </div>
        ...
        <script>
            ...
            function updateStatus() {
                ...
                document.getElementById('status-info').innerHTML = \`
                    <div class="space-y-2">
                        ...
                        <p><strong>Token guardado:</strong> \${existing ? '[SUCCESS] SÍ' : '[ERROR] NO'}</p>
            ...
            async function quickLogin() {
                const results = document.getElementById('results');
                results.innerHTML = '<div class="animate-pulse">[REFRESH] Haciendo login...</div>';
                    ...
                    if (response.data.success) {
                        ...
                        results.innerHTML = \`
                            <div class="text-green-600">
                                <h3 class="font-semibold mb-2">[SUCCESS] Login Exitoso</h3>
                        ...
                        results.innerHTML = \`<div class="text-red-600">[ERROR] Error: \${response.data.error}</div>\`;
                    }
                } catch (error) {
                    console.error('Login error:', error);
                    results.innerHTML = \`<div class="text-red-600">[ERROR] Error de conexión: \${error.message}</div>\`;
                }
            }
            ...
            async function testMonitoringDirect() {
                ...
                if (!adminToken) {
                    results.innerHTML = '<div class="text-orange-600">[WARNING] Necesitas hacer login primero</div>';
                    return;
                }
                
                results.innerHTML = '<div class="animate-pulse">[REFRESH] Probando API de monitoreo...</div>';
                ...
                try {
                    ...
                    results.innerHTML = \`
                        <div class="text-green-600">
                            <h3 class="font-semibold mb-2">[SUCCESS] API Monitoreo Funcionando</h3>
                    ...
                    results.innerHTML = \`
                        <div class="text-red-600">
                            <h3 class="font-semibold mb-2">[ERROR] Error en API</h3>
            ...
            function loadFullDashboard() {
                if (!adminToken) {
                    document.getElementById('results').innerHTML = '<div class="text-orange-600">[WARNING] Necesitas hacer login primero</div>';
                    return;
                }
                window.location.href = '/dashboard';
            }
            ...
            <div class="bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6 mt-8">
                ...
                <div class="mt-6 p-4 bg-white/50 dark:bg-black/20 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                            <p class="font-medium text-red-600 mb-2">[ERROR] Dashboard Original:</p>
                        ...
                        <div>
                            <p class="font-medium text-green-600 mb-2">[SUCCESS] Dashboard Mejorado:</p>
                            <ul class="space-y-1 text-muted-foreground">
  ...
  return c.html(`
    ...
    <body class="bg-gray-100 p-8">
        <div class="max-w-4xl mx-auto space-y-6">
            ...
            <!-- Login Section -->
            <div class="bg-white p-6 rounded-lg shadow">
                <h1 class="text-2xl font-bold mb-6">[TOOL] Debug Dashboard - Test Rápido</h1>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h2 class="text-lg font-semibold mb-4">[KEY] Login</h2>
                        <div class="space-y-3">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold mb-4">[DATA] Dashboard</h2>
                        <div class="space-y-3">
            ...
            <!-- Status Panel -->
            <div class="bg-white p-6 rounded-lg shadow">
                <h2 class="text-xl font-semibold mb-4">[INFO] Estado del Sistema</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="text-center">
                    ...
                    <div class="text-center">
                        <div class="text-2xl mb-2">[DATA]</div>
        ...
        <script>
            ...
            function updateStatus() {
                const token = localStorage.getItem('auth_token');
                document.getElementById('token-status').innerText = token ? '[SUCCESS] Presente' : '[ERROR] Ausente';
                
                if (token) {
                    try {
                        const payload = JSON.parse(atob(token.split('.')[1]));
                        document.getElementById('user-status').innerText = \`[SUCCESS] \${payload.email}\`;
                        log(\`Token válido para: \${payload.email} (\${payload.role})\`, 'success');
                    } catch (e) {
                        document.getElementById('user-status').innerText = '[WARNING] Token inválido';
            ...
            async function createUser() {
                ...
                try {
                    ...
                    if (result.success) {
                        showResult('login-result', '[SUCCESS] Usuario administrador creado! Ahora puedes hacer login.');
                        log('Usuario creado exitosamente', 'success');
                    } else {
                        showResult('login-result', \`[ERROR] Error creando usuario: \${result.error}\`, false);
                        log(\`Error creando usuario: \${result.error}\`, 'error');
                    }
                } catch (error) {
                    showResult('login-result', \`[ERROR] Error: \${error.message}\`, false);
            ...
            async function doLogin() {
                ...
                try {
                    ...
                    if (response.data.success && response.data.data.token) {
                        ...
                        showResult('login-result', '[SUCCESS] Login exitoso! Token guardado correctamente.');
                        log('Login exitoso, token guardado', 'success');
                        updateStatus();
                    } else {
                        showResult('login-result', '[ERROR] Login falló: Respuesta inválida', false);
                        log('Login falló: respuesta sin token', 'error');
                    }
                } catch (error) {
                    const errorMsg = error.response?.data?.error || error.message;
                    showResult('login-result', \`[ERROR] Error: \${errorMsg}\`, false);
            ...
            async function testDashboard() {
                ...
                const token = localStorage.getItem('auth_token');
                if (!token) {
                    showResult('dashboard-result', '[ERROR] No hay token. Haz login primero.', false);
                ...
                try {
                    ...
                    document.getElementById('dash-status').innerText = '[SUCCESS] Funcional';
                    showResult('dashboard-result', '[SUCCESS] Dashboard funciona correctamente!');
                    
                } catch (error) {
                    const errorMsg = error.response?.data?.error || error.message;
                    document.getElementById('dash-status').innerText = '[ERROR] Error';
                    showResult('dashboard-result', \`[ERROR] Error: \${errorMsg}\`, false);
